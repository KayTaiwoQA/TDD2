...
public class Questions {
	...
	public int returnPosition(String input, char letter) {
		...
		int f = string1.indexOf(letter) + 1;
		if (f > 0) {
			return f;
		} else {
			return -1;
	...
	public String[] removeDuplicates(String[] stringArray) {
		...
		String[] objects = {"Dog","Cat","Dog"};
		objects = Arrays.stream(stringArray).distinct().toArray(String[]::new);
		stringArray = objects;
	...
	public int[] listGen() {
		...
		for (int i = 0; i < 5; i++) {
			//double z = Math.random().nextDouble(100, 200 + 1);
			randnum= ThreadLocalRandom.current().nextInt(100, 200 + 1);
			arr[i] = randnum;
		}
		return arr;
	...
	public int getVowel(String input) {
		...
		for (v = 0; v < input.length(); v++) {
			char cha = input.charAt(v);
			String chara = String.valueOf(cha);
			if (chara.equals("a") || chara.equals("e") || chara.equals("i") || chara.equals("u") || chara.equals("o")) {
				counter++;
			}
		}
		return counter;
